...
public class Socks5Impl extends Socks4Impl {
    ...
    public InetAddress calcInetAddress(byte AType, byte[] addr) {
        ...
        switch (AType) {
            ...
            case 0x03:
                if (addr[0] <= 0) {
                    Log.d("AAA", "SOCKS 5 - calcInetAddress() : BAD IP in command - size : " + addr[0]);
                    return null;
                }
    ...
    public void refuseAuthentication(String msg) {
        Log.d("AAA", "SOCKS 5 - Refuse Authentication: '" + msg + "'");
        m_Parent.sendToClient(SRE_REFUSE);
    }


    public void acceptAuthentication() {
        Log.d("AAA", "SOCKS 5 - Accepts Auth. method 'NO_AUTH'");
    ...
    public void getClientCommand() throws Exception {
        ...
        if (SOCKS_Version != SocksConstants.SOCKS5_Version) {
            Log.d("AAA", "SOCKS 5 - Incorrect SOCKS Version of Command: " +
                    SOCKS_Version);
        ...
        if ((socksCommand < SocksConstants.SC_CONNECT) || (socksCommand > SocksConstants.SC_UDP)) {
            Log.d("AAA", "SOCKS 5 - GetClientCommand() - Unsupported Command : \"" + commName(socksCommand) + "\"");
            refuseCommand((byte) 0x07);
            throw new Exception("SOCKS 5 - Unsupported Command: \"" + socksCommand + "\"");
        }

        if (ADDRESS_TYPE == 0x04) {
            Log.d("AAA", "SOCKS 5 - GetClientCommand() - Unsupported Address Type - IP v6");
            refuseCommand((byte) 0x08);
            throw new Exception("Unsupported Address Type - IP v6");
        }

        if ((ADDRESS_TYPE >= 0x04) || (ADDRESS_TYPE <= 0)) {
            Log.d("AAA", "SOCKS 5 - GetClientCommand() - Unsupported Address Type: " + ADDRESS_TYPE);
            refuseCommand((byte) 0x08);
        ...
        Log.d("AAA", "SOCKS 5 - Accepted SOCKS5 Command: \"" + commName(socksCommand) + "\"");
    }

    public void replyCommand(byte replyCode) {
        Log.d("AAA", "SOCKS 5 - Reply to Client \"" + replyName(replyCode) + "\"");
    ...
    public void bindReply(byte replyCode, InetAddress IA, int PT) {
        ...
        Log.d("AAA", "BIND Reply to Client \"" + replyName(replyCode) + "\"");
        ...
        if (m_Parent.isActive()) {
            ...
            Log.d("AAA", "BIND - Closed Client Connection");
        }
    }

    public void udpReply(byte replyCode, InetAddress IA, int pt) {
        Log.d("AAA", "Reply to Client \"" + replyName(replyCode) + "\"");

        if (m_Parent.m_ClientSocket == null) {
            Log.d("AAA", "Error in UDP_Reply() - Client socket is NULL");
        }
    ...
    public void udp() throws IOException {
        ...
        Log.d("AAA", "UDP Listen at: <" + MyIP.toString() + ":" + MyPort + ">");

        while (m_Parent.checkClientData() >= 0) {
            processUdp();
            Thread.yield();
        }
        Log.d("AAA", "UDP - Closed TCP Master of UDP Association");
    }
    ...
    private byte[] clearDgpHead(byte[] buffer) {
        ...
        switch (AType) {
            ...
            default:
                Log.d("AAA", "Error in ClearDGPhead() - Invalid Destination IP Addres type " + AType);
                return null;
        }
        ...
        if (UDP_IA == null) {
            Log.d("AAA", "Error in ClearDGPHead() - Invalid UDP dest IP address: NULL");
            return null;
        }
    ...
    protected void udpSend(DatagramPacket DGP) {
        if (DGP != null) {
            ...
            try {
                ...
                Log.d("AAA", "Error in ProcessUDPClient() - Failed to Send DGP to " + LogString);
            }
        }
    }
    ...
    public void processUdp() {
        ...
        try {
            ...
            Log.d("AAA", "Error in ProcessUDP() - " + e.toString());
            return;
        }
        ...
        try {
            ...
            Log.d("AAA", "IOError in Init_UDP_IO() - " + e.toString());
            m_Parent.close();
        }
    }
    ...
    public void processUdpClient() {
        ...
        if (UDP_IA == null) {
            Log.d("AAA", "Error in ProcessUDPClient() - Invalid Destination IP - NULL");
            return;
        }
        if (UDP_port == 0) {
            Log.d("AAA", "Error in ProcessUDPClient() - Invalid Destination Port - 0");
            return;
        }
        ...
        Log.d("AAA", "Datagram : " + Buf.length + " bytes : " + getSocketInfo(DGPack) +
    ...
    public void processUdpRemote() {
        Log.d("AAA", String.format("Datagram : %d bytes : <%s:%d> << %s",
